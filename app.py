...
# 図の編集（sao_graph_editor）。フォルダ sao_graph_editor（vis-network を同梱）があればそれを使い、
# 無ければ（app.py だけを置いた場合・Streamlit Cloud にフォルダを上げていない場合）、下の HTML を一時フォルダに書き出し、
# vis-network は CDN から読む。どちらも使えないときは、図の編集だけを止めて、ほかの画面は動かす。
_GRAPH_EDITOR_HTML = r'''<!DOCTYPE html>
<html lang="ja">
<head>
<meta charset="utf-8">
...
// vis-network を読む（同じフォルダ → jsDelivr → unpkg の順）。読めなければ、その旨を表示する
function loadVis(urls, done) {
  if (!urls.length) { done(false); return; }
  const sc = document.createElement("script"); sc.src = urls[0];
  sc.onload = () => done(true); sc.onerror = () => loadVis(urls.slice(1), done);
...
loadVis(["vis-network.min.js",
         "https://cdn.jsdelivr.net/npm/vis-network@9.1.9/standalone/umd/vis-network.min.js",
         "https://unpkg.com/vis-network@9.1.9/standalone/umd/vis-network.min.js"], (ok) => {
  if (!ok) {
    document.body.innerHTML = '<div style="padding:14px;border:1px solid #f59e0b;border-radius:8px;background:#fffbeb;color:#92400e;font-size:14px">' +
      '図を描く部品（vis-network）を読み込めませんでした。app.py と同じ場所に sao_graph_editor フォルダを置くか、インターネットにつながる環境で開いてください。上の表では直せます。</div>';
    send("streamlit:componentReady", {apiVersion: 1});
    send("streamlit:setFrameHeight", {height: 90});
    return;
  }
  main();
  send("streamlit:componentReady", {apiVersion: 1});
...
def _make_graph_editor():
    import hashlib
    import tempfile
    try:
        d = APP_DIR / "sao_graph_editor"
        if not ((d / "index.html").exists() and (d / "vis-network.min.js").exists()):
            d = Path(tempfile.gettempdir()) / ("sao_graph_editor_" + hashlib.sha1(_GRAPH_EDITOR_HTML.encode("utf-8"))
                                               .hexdigest()[:10])
            d.mkdir(parents=True, exist_ok=True)
            (d / "index.html").write_text(_GRAPH_EDITOR_HTML, encoding="utf-8")
        return components.declare_component("sao_graph_editor", path=str(d))
    except Exception:  # noqa: BLE001
        return None


_GRAPH_EDITOR = _make_graph_editor()
...
def graph_edit_section(edited, kb):
    ...
    if _GRAPH_EDITOR is None:
        st.warning("図の編集の部品を読み込めませんでした（app.py と同じ場所に sao_graph_editor フォルダを置くと使えます）。"
                   "上の表で直してください。")
        return sao_graph([dict(r) for r in rels], key="analyze")
